                        ...
                        <div id="report-filters">
                            <form action="{{ route('reports.generate') }}" method="GET" class="row align-items-end">
                                <div class="col-md-12 mb-4 d-flex justify-content-end">
                                    {{-- <label class="d-block text-muted small gap-1">Quick Filters:</label> --}}
                                    <div class="btn-group btn-group-sm" role="group" style="display: block;">
                                        ...
                                        <button type="button" class="btn btn-gradient-secondary btn-lg"
                                ...
                                <div class="col-md-4 form-group">
                                    ...
                                    <select name="report_type" class="form-control form-control-sm">
                                        ...
                                        <option value="settlements"
                                            {{ isset($reportType) && $reportType == 'settlements' ? 'selected' : '' }}>
                                            Settlements (Shifts)</option>
                                        <option value="stock"
                                            {{ isset($reportType) && $reportType == 'stock' ? 'selected' : '' }}>Stock
                                            Movement</option>
                                        <option value="cash_drops"
                                            {{ isset($reportType) && $reportType == 'cash_drops' ? 'selected' : '' }}>Cash
                                            Drops</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-2 form-group">
                                    <button type="submit" class="btn btn-gradient-primary btn-md w-100">Generate</button>
                                </div>
                            </form>
                        </div>
                    </div>
                                ...
                                <div class="row mb-4">
                                    @if ($reportType === 'sales')
                                        <div class="col-md-6 stretch-card grid-margin">
                                            <div class="card bg-gradient-success card-img-holder text-white">
                                                <div class="card-body">
                                                    <h4 class="font-weight-normal mb-3">Total Sales Amount <i
                                                            class="mdi mdi-currency-usd mdi-24px float-right"></i></h4>
                                                    <h2 class="mb-5">{{ number_format($totals['total_amount'], 2) }}
                                                    </h2>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-6 stretch-card grid-margin">
                                            <div class="card bg-gradient-info card-img-holder text-white">
                                                <div class="card-body">
                                                    <h4 class="font-weight-normal mb-3">Total Quantity <i
                                                            class="mdi mdi-gas-station mdi-24px float-right"></i></h4>
                                                    <h2 class="mb-5">{{ number_format($totals['total_quantity'], 2) }}
                                                    </h2>
                                                </div>
                                            </div>
                                        </div>
                                    @elseif($reportType === 'settlements')
                                        ...
                                        <div class="col-md-4 stretch-card grid-margin">
                                            <div class="card bg-gradient-success card-img-holder text-white">
                                                <div class="card-body">
                                                    <h4 class="font-weight-normal mb-3">Cash Sales <i
                                                            class="mdi mdi-cash mdi-24px float-right"></i></h4>
                                                    <h2 class="mb-5">{{ number_format($totals['cash_sales'] ?? 0, 2) }}
                                                    </h2>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="col-md-4 stretch-card grid-margin">
                                            <div
                                                class="card {{ $totals['total_variance'] < 0 ? 'bg-gradient-danger' : 'bg-gradient-success' }} card-img-holder text-white">
                                                <div class="card-body">
                                                    <h4 class="font-weight-normal mb-3">Total Variance <i
                                                            class="mdi mdi-alert-circle-outline mdi-24px float-right"></i>
                                                    </h4>
                                                    <h2 class="mb-5">{{ number_format($totals['total_variance'], 2) }}
                                                    </h2>
                                                </div>
                                            </div>
                                        </div>
                                    @elseif($reportType === 'cash_drops')
                                        <div class="col-md-12 stretch-card grid-margin">
                                            <div class="card bg-gradient-success card-img-holder text-white">
                                                <div class="card-body">
                                                    <h4 class="font-weight-normal mb-3">Total Cash Drops <i
                                                            class="mdi mdi-cash-multiple mdi-24px float-right"></i></h4>
                                                    <h2 class="mb-5">{{ number_format($totals['total_amount'], 2) }}
                                                    </h2>
                                                </div>
                                            </div>
                                    ...
                                    @elseif($reportType === 'stock')
                                        <div class="col-md-12 stretch-card grid-margin">
                                            <div class="card bg-gradient-info card-img-holder text-white">
                                                <div class="card-body">
                                                    <h4 class="font-weight-normal mb-3">Total Stock Movement <i
                                                            class="mdi mdi-cube-outline mdi-24px float-right"></i></h4>
                                                    <h2 class="mb-5">{{ number_format($totals['total_quantity'], 2) }}
                                                    </h2>
                                                </div>
                                            </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            @forelse($reportData as $row)
                                                <tr>
                                                    ...
                                                    @elseif($reportType === 'settlements')
                                                        ...
                                                        <td>{{ number_format($row->total_sales, 2) }}</td>
                                                        <td
                                                            class="{{ $row->cash_variance < 0 ? 'text-danger' : 'text-success' }}">
                                                            {{ number_format($row->cash_variance, 2) }}
                                                        </td>
                                                        <td>
                                                            <label
                                                                class="badge {{ $row->status === 'closed' ? 'badge-success' : 'badge-warning' }}">
                                                                {{ ucfirst($row->status) }}
                                                            </label>
                                                        </td>
                                                    @elseif($reportType === 'stock')
                                                        ...
                                                        <td>
                                                            <label
                                                                class="badge {{ $row->type === 'in' ? 'badge-success' : 'badge-danger' }}">
                                                                {{ ucfirst($row->type) }}
                                                            </label>
                                                        </td>
                                                        <td>{{ number_format($row->quantity, 2) }}</td>
                                                        <td>{{ $row->reference_number ?? '-' }}</td>
                                                    @elseif($reportType === 'cash_drops')
                                                        <td>{{ \Carbon\Carbon::parse($row->dropped_at)->format('Y-m-d H:i') }}
                                                        </td>
                                                        <td>{{ $row->user->name ?? '-' }}</td>
                                                        <td>{{ $row->shift->shift_number ?? '-' }}</td>
                                                        <td>{{ number_format($row->amount, 2) }}</td>
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="8" class="text-center text-muted">No records found.
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
